...
class NodeDataSet(data.Dataset):
    ...
    def __init__(self,train=False,test=False,val=False):
        ...
        # 将dcm和mask一一对应
        mask_np = np.zeros((0, opt.dcm_h,opt.dcm_w))  # 以便合并，该tensor无用  512为CT的长宽
    ...
    def __getitem__(self, index):
        ...
        return img,mask   # mask为 肌肉+背景，所以任务是分割肌肉
    ...
    def Pretreatment(self,dcm_dir):
        '''
        仅保留中间连通区域的腹窗图像  (在预处理保存所有dcm数据为list   tudo)
        '''
        # 读取dcm
    ...
    def DataAugmentation(self,img_np,mask_np):
        ...
        # 合并img和mask，统一增强
        mix_np=np.concatenate((np.expand_dims(img_np, axis=0), np.expand_dims(mask_np, axis=0)), axis=0) # [2,512,512]
