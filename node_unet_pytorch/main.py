...
import numpy as np
from utils.getRecall import getRecall
...
def train():
    ...
    test_dataloader = DataLoader(test_data, opt.test_batch_size, shuffle=False, num_workers=opt.num_workers)
    for epoch in range(opt.max_epoch - start_epoch):
        ...
        if epoch % 10 == 0:
            ...
            for kk, (img_test, mask_test) in enumerate(test_dataloader):
                ...
                probs = t.sigmoid(mask_pred_test).squeeze().squeeze().cpu().detach().numpy()  # [512,512]
...
def predict():
    ...
    for ii, full_img in enumerate(test_dataloader):
        ...
        # 保存mask为npy
        np.save('/home/bobo/data/test/test8/' + str(10000 + ii) + '_mask.npy', mask)
